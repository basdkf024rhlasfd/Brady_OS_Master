...
- **Musashi Review:** `0-agents/custom-built-agents/musashi-SKILL.md` — Daily midnight CT agent tension pass. Scores every custom agent on 5 objective dimensions (Activation / Output Landed / Autonomy / Trigger Clarity / Surprise Value, 0–2 each, /10 total), emits 1–3 recommendations per below-threshold agent, scans the web via Exa + Bright Data for brand-new AI tools / MCPs / platforms, and generates 3–5 low-manual-lift biz ideas. Writes gitted backup to `1-execution/areas/brady-os/musashi-reviews/YYYY-MM-DD.md` + a `Type="Musashi Review"` row in Streaming Notes that morning sweep Phase 1.0c consumes. **Every item has an approval slug — nothing auto-executes without Brady's `approve musashi [slug]` reply.** The daily tension cycle: Musashi proposes → morning sweep sizes → Brady approves → builder executes → next night Musashi re-scores. Executes as Claudine-tier SOP. Trigger: "run musashi", "musashi review", "agent review", "run the tension cycle".
