...
- `1-execution/` — Areas > Programs > Projects > Tasks. Areas: `brady-os` (gitted backups: audits, commissioner-briefs, hygiene-heidi-reports, musashi-reviews, phil-morning-audits, processing-scores, project-agent-standups), `family`, `work-and-business` (consulting projects live here).
- `2-memory/` — Unstructured intake (lives in Notion, not repo).
- `3-reference/` — Rules, governance, skills, publishing. Standalone reusable skills go in `3-reference/skills/<name>/`. Also: `build-queue/` (SPEC-NNN build specs + INDEX.md), `patterns/`, `scripts/`, `imported-skills-and-systems/`.
- `portal/` — mception.ai Next.js app (Clerk auth, project portal). Deployed to Vercel project **`mception-ai`** (the "munich" project is legacy — do not use). Config: `portal/src/config/projects.yml`. Musashi San owns all deploy + env-var operations here (via `webster-SKILL.md` sub-routines).

Supporting top-level items (not layers):
- `docs/investigations/` — Audit and investigation write-ups (KB audit, repo cleanup, removed-artifact manifests).
- `os-cockpit/`, `os-viewer/` — Static HTML dashboards. `os-cockpit/data.js` is gitignored (PII/financials); `data.example.js` is the committed template.
- `output/` — Generated workshop artifacts. Sources (.html/.md) are tracked; PDFs and `images*/` dirs are gitignored (regenerable).
- `references/learning-log.yml` — Recursive learning log.
- `AGENTS.md` — Human-readable pointer; core repo rules live here in CLAUDE.md.

## Development Workflow & Conventions
- **Portal dev:** `cd portal && npm install && npm run dev` (localhost:3000). Build/lint: `npm run build`, `npm run lint`. Env: copy `.env.example` → `.env.local` with Clerk keys; auth model documented in `portal/docs/clerk-primer.md`. For fast local UI iteration with auth bypass, use the `mception-local-dev` skill.
- **Portal layout:** protected pages under `portal/src/app/(portal)/`, public under `(public)`; per-project chat configs in `portal/src/config/chat-configs/*.yml`; sidebar groups in `portal/src/lib/sidebar-groups.ts`; project registry (access control + magic links + iframe config) in `portal/src/config/projects.yml`.
- **Sensitive data never in git:** family data, Monarch CSVs, `os-cockpit/data.js`, and school calendars are gitignored. Real client financials go to `~/brady-os-local/{client-slug}-confidential/`; kids' records to `~/brady-os-local/family/kids/`; financial actuals to `~/brady-os-local/finance/`.
- **Git coordination:** multiple harnesses (Conductor, Dispatch/Cowork, Claude Code) share this repo. Load `3-reference/skills/dispatch-git-protocol/SKILL.md` before file writes from Dispatch. Branch ownership map and push gates live there.
- **Generated artifacts:** don't commit workshop renders — `3-reference/skills/*/output/*.pdf` and `output/images*/` are gitignored; keep the .html/.md sources.
...
- `3-reference/connector-registry.yml` — Canonical per-harness inventory of MCPs, CLIs, browser surfaces, and data sources with `verify:` probes. **Load at session start. When any harness claims "I don't have X," check this first before re-installing.** Maintained nightly by Musashi (Connector Scout — Phase 4.5) and weekly by Hygiene Heidi (Rule 6).
...
- **mception Design System:** `3-reference/skills/mception-design-system/SKILL.md` — Canonical design language for all visual docs, infographics, briefings. Dark-mode-first, CEO-scannable.
- **Daily Decision Queue:** `3-reference/skills/daily-decision-queue/SKILL.md` — **Phase 0 of morning sweep.** Aggregates every pending `approve [agent] [slug]` from Musashi, Heidi, Phil, project-agent standups, and Streaming Notes System Instructions into ONE numbered list. Brady replies once: `approve 1,2,5 | reject 3 | change 4: smaller scope`. Owns the dream-state daily approval surface. Trigger: "decision queue", "open the day", "what needs approval", "pending decisions".
- **Morning Sweep:** `3-reference/skills/morning-sweep/SKILL.md` — Brady's canonical morning sweep across Gmail, iMessage, Notion, Calendar, Otter, and family logistics. **Now starts with Phase 0 (Decision Queue) and Phase 0.5 (Claudine's Daily Money Check, replacing the old daily Finn block).**
...
- **Brain Dump Triage:** `3-reference/skills/brain-dump-triage/SKILL.md` — On-demand processor for big unsorted dumps (Apple Reminders, voice notes, scratch lists) when there's too much in active memory and the lift of organizing alone is the blocker. Reads Reminders directly via SQLite (bypasses TCC wall), waits for Otter/typed disposition guidance, then builds a stress-reducing **Notion checklist with H3 toggles** for one-section-at-a-time work + creates calendar events for time-locked items + drafts texts for delegations + saves a `.context/reminder-triage-YYYY-MM-DD.md` copy. Trigger: "brain dump", "active memory dump", "got too much in my head", "process my reminders", "triage my list", "drain my reminders".
...
- **Budget Guidance (Finn):** `3-reference/skills/budget-guidance/SKILL.md` — Finn's objective budgeting engine (added 2026-07-16). Turns lumpy 1915 South base+bonus income into a steady smoothed "bonus salary," runs four buckets (Core/Living/Build/Reserve) against traffic-light thresholds, enforces the spend waterfall, and answers "can we afford X?" deterministically. Owns the food-line lever ($4,500/mo, $1,040/wk), the HELOC-is-not-income rule, and the bonus-landing + monthly-close protocols. Numbers in `budget-targets.md`; sensitive actuals in `~/brady-os-local/finance/`. Trigger: "budget", "can we afford", "budget check", "bonus landed", "are we on track", "budget close".
...
- **Project Agent Standup:** `3-reference/skills/project-agent-standup/SKILL.md` — Daily 5-phase agile ceremony for project agents (design thinking, OKR review, sprint planning, sprint review, service model tension). Creates natural inertia toward better engagement models. Runs 5 AM CT, feeds morning sweep. Starting roster: OC Optimus (Engagement Architect) + Fran (Engagement Accelerator). Gitted backup: `1-execution/areas/brady-os/project-agent-standups/`
...
- **Innovation Workshop (PRODUCT side):** `3-reference/skills/innovation-workshop/SKILL.md` — **Product** innovation engine. Trend-first Stage 0, 20 ideas per run, Canva/Midjourney visuals, research-backed pitches, RTBs, executive summary, whitepaper appendix. Recursive learning engine tracks method performance. Output: HTML + PDF. **Advanced-model execution layer (2026-07-18):** parallel blind method fan-out, 3-judge scoring panel (Margin Realist / Novelty Maximalist / Ops Skeptic), adversarial verification of comparables/cost models/RTBs, MJ image QA pass, computed cross-run analytics with approval-gated policy recommendations, full-auto as a supported nightly mode, Stage 3 Light Pass. For OPS innovation use `operations-innovation-engine` instead. If Brady says just "innovation workshop" without "product" or "ops," ask which engine.
- **Operations Innovation Engine (OPS side):** `3-reference/skills/operations-innovation-engine/SKILL.md` — **Ops** innovation engine. Problem-first Stage 0 (P1-PN problem statements), 25-35 tiered concepts per run, no visuals (text + tables only), operator wisdom library (~20 cards: TOC, VSM, PR/FAQ, Hoshin Kanri, Chipotle, CFA, Foran), pilot specs, labor/throughput/scalability scoring rubric. Inaugural run: Panda (QSR ops). Disambiguation partner of innovation-workshop.
...
- **Mason:** `0-agents/custom-built-agents/mason.md` — System packager and rebuild architect. Turns the live OS into friend-safe rebuild guides, starter kits, and public explainers without leaking private internals. Reads canonical files, never packages from memory. Artifact classes: public overview / rebuild guide / activation kit.
- **Yuki Ronin:** `0-agents/custom-built-agents/yuki-ronin.md` — Masterless builder; executes specs with precision, does not strategize. Reports to Musashi San; domain questions route to the relevant Domain Expert. Modes: Build (default) / Scale (Fubuki-tai squad) / Review. Ronins named A-E with persistent domain ownership, up to 5 active.
- **Phil Pre-Sweep:** `0-agents/custom-built-agents/phil-SKILL.md` — Daily 4 AM Notion grooming + morning-sweep primer. Reconciles Done/Status mismatches (autonomous, capped at 20/run), surfaces carryover / 7-day horizon / coherence flags in Phil's voice, proposes TOP 3 candidates. Writes gitted backup to `1-execution/areas/brady-os/phil-morning-audits/YYYY-MM-DD.md` + a `Type="Pre-Sweep Primer"` row in Streaming Notes that morning sweep Phase 1.0b consumes. Executes as Claudine-tier SOP — Phil the ChatGPT agent remains non-executing per Amendment 1. Trigger: "run phil", "phil sweep", "phil audit", "prime the sweep", "groom notion".
- **Musashi San:** `0-agents/custom-built-agents/musashi-SKILL.md` — Two-mode skill: (1) nightly agent review + tension pass (midnight CT), and (2) deploy authority (on-demand). Review mode: scores every custom agent on 5 objective dimensions, emits recommendations, scans for new AI tools/MCPs, generates biz ideas. Deploy mode: owns all mception.ai publishing, Vercel env vars, Clerk allowlists, failed deploy diagnostics, and UAT via `webster-SKILL.md` sub-routines. Every review recommendation is approval-gated (`approve musashi [slug]`). Tension cycle: Musashi proposes → morning sweep sizes → Brady approves → builder executes → next night Musashi re-scores. Deploy triggers: "publish to mception", "deploy [X]", "add access for [email]", "fix the build", "wire up [API]", or any web/deploy ops. Review triggers: "run musashi", "musashi review", "agent review", "run the tension cycle". ARCHIVED: `webster.md` (persona absorbed 2026-04-27). Runbooks: `0-agents/custom-built-agents/webster-SKILL.md`.
...
- **mception Fast Path:** `3-reference/skills/mception-fast-path/SKILL.md` — One-line control of mception.ai publishing + access with guardrails baked in (least privilege, fail closed, protected-surface confirms, mandatory UAT, receipts, audit trail). Grammar: "publish [slug] for [email]", "grant/revoke [email] on [slug]", "who sees [slug]", "access audit", "family page [name]". Mechanics execute via Musashi deploy mode (webster runbooks). Named page-set `family-shared` = family-budget + grocery-assistant + school-hub. (The portal's Family sidebar group additionally shows `healthcare` — benefits handbook for Karissa, added 2026-07-16 — plus `financial-assistant` and `bucket-system`; those are NOT part of the `family-shared` grant set.)
...
- **Conductor Push:** `3-reference/skills/conductor-push/SKILL.md` — Send Telly/Telegram messages directly from Conductor via bot token (no push secret needed), and schedule delayed one-shot tasks via ScheduleWakeup. Trigger: "message me from telly", "in X minutes do Y", "delay this", "schedule this for later".
- **Roborock Control:** `3-reference/skills/roborock-control/SKILL.md` — Drive Brady's Roborock vacuum via the Home Assistant REST API (HA in Docker on Mac at localhost:8123, Roborock integration). Primitives: start, pause, stop, return-to-base, locate, set-fan-speed, segment-clean, status. `~/homeassistant/roborock.sh` wrapper + cron-friendly. Trigger: "send the vacuum home", "vacuum [room]", "roborock status", "is the vacuum charged".
- **Dispatch Git Protocol:** `3-reference/skills/dispatch-git-protocol/SKILL.md` — Git coordination rules for Dispatch (CoWork/claude.ai) operating alongside Conductor. Branch ownership map, push gates, merge checklist, conflict avoidance, Telly signaling. Load before any file writes in brady-os. Trigger: "dispatch protocol", or load at start of any Dispatch session touching the repo.
- **Agent Scheduler:** `3-reference/skills/agent-scheduler/SKILL.md` — SOP for creating new scheduled agents following Brady OS conventions (Phil/Finn pattern). Gathers inputs, produces structured SKILL.md scaffold, CLAUDE.md registry line, scheduling checklist, and first-run verification protocol. Trigger: "create a scheduled agent", "set up a new scheduled skill", "I want something to run every night at X", "build me a [name] agent that runs at [time]".
- **Kids Test Results 1-Pager:** `3-reference/skills/kids-test-results-onepager/SKILL.md` — Print-ready, age-appropriate 1-pager per kid based on standardized test results (ATLAS, NWEA MAP, etc.). Specific praise grounded in actual scores, calibrated reading list (by Lexile band), workbook recs (push-ahead + targeted-to-weakness), kid-specific closing line. HTML template at `templates/onepager-template.html`. Color identity: Isla=purple, Luke=blue, Quinn=pink, Lily=green, Faith=amber. Real outputs land in `~/brady-os-local/family/kids/` (sensitive data, never repo). Trigger: "make a 1-pager for the kids' [test] results", "summer plan one-pager", "kid-friendly test results".
- **Broker Platform:** `3-reference/skills/broker-platform/SKILL.md` — Full agent suite for CPG/food retail brokers. 7 agents: Brand Intake, Buyer Intel, Pitch Builder, Category Watch, Meeting Prep, Call Notes, Pipeline Dashboard. Standalone Vercel app + mception.ai/broker. Brady OS methodology infused. V1 scope: Brand Intake + Buyer Intel + Pipeline Dashboard.
- **Consulting OS Platform:** `3-reference/skills/consulting-os-platform/SKILL.md` — Full agent suite for independent strategy consultants. 7 agents: Engagement Scoper, Research Agent, Framework Router, Deliverable Generator, Client Comms, Engagement Health, Revenue Dashboard. Standalone Vercel app + mception.ai/consulting-os. Brady's management OS as the core IP. V1 scope: Engagement Scoper + Research Agent + Revenue Dashboard.
- **Claudine Scorecard:** `3-reference/skills/claudine-scorecard/SKILL.md` — Objective weekly scorecard replacing subjective Jarvis Score. 16 instrumented KPIs + North Star (Hands-Off Index) + K16 Research Score (Indexed Reports + Project Coverage + Leverage, composite /10). Every metric has a formula, data source, and scoring band pulled from Streaming Notes, git, Finn, Phil, Musashi, Research Library. Runs weekly via weekly-sweep or on demand ("score claudine"). Auto-proposes fixes for any metric scoring <5.
- **Client P&L Due Diligence:** `3-reference/skills/client-pnl-dd/SKILL.md` — Reusable DD workflow for client/target-company P&L files. Parses .xls/.xlsx, reconciles multi-version ledgers (shadow P&L diagnostic), extracts trend history, ranks store/segment performance, produces a DD memo. Confidentiality default: client financials live at `~/brady-os-local/{client-slug}-confidential/pnl-dd-YYYY-MM/`, never in git. Trigger: "P&L diligence", "run DD on these financials", "shadow P&L bridge". First run: 1915 South 2026-05-11 — found $3.55M shadow gap on 2024 exec comp basis.
- **Hygiene Heidi:** `0-agents/custom-built-agents/hygiene-heidi-SKILL.md` — Weekly Saturday 8 AM CT OS compliance check. Audits every agent against **7 canonical hygiene rules** (objective scoring methodology, self-scoring, improvement-seeking, Streaming Notes purgatory ≤7 days, **Research Library health — K16 ≥5/10, no items unreferenced >90d, every active client ≥10 sources**, **Connector Registry — every entry verified ≤30 days with passing probe, no undocumented connectors**, **Repo hygiene — `3-reference/scripts/repo-janitor/repo-janitor.sh` full sweep passes: no oversized files, build artifacts, family binaries, or duplicate bloat; same script gates PRs via `.github/workflows/repo-janitor.yml`**). Red/amber/green brief + approval gates for all violations. Rules 5 & 6 recomputed live every Saturday run. Writes gitted backup at `1-execution/areas/brady-os/hygiene-heidi-reports/YYYY-MM-DD.md` + Streaming Notes row + Routing Log row + `last_verified:` timestamp updates in `connector-registry.yml`. Trigger: "run heidi", "hygiene check", "compliance check".
