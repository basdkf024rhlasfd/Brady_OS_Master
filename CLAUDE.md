...
- **Musashi UAT:** `0-agents/custom-built-agents/musashi-uat-SKILL.md` — Mandatory image-bearing-surface UAT sub-skill. Before any rendering pipeline (innovation-workshop, exec-intel-brief, daily-whitepaper, deck-generator, presentation-engine, infographic-builder, marketing-templates, client-engagement-kit, content-publishing-kit) is "done", open the actual final surface in Chrome MCP, screenshot it, walk every hero image visually, and confirm metadata. File size + git commit + tool-success messages do NOT prove visible images. Catalogues failure modes from the 2026-04-29 Run 7 incident (empty Notion Image URL field, square-box CSS clipping panoramic MJ output, stale Run option label). Triggers: "uat this", "verify the images", "musashi uat".
...
- **Hygiene Heidi:** `0-agents/custom-built-agents/hygiene-heidi-SKILL.md` — Weekly Saturday 8 AM CT OS compliance check. Audits every agent against **5 canonical hygiene rules** (objective scoring methodology, self-scoring, improvement-seeking, Streaming Notes purgatory ≤7 days, **Research Library health — K16 ≥5/10, no items unreferenced >90d, every active client ≥10 sources**). Red/amber/green brief + approval gates for all violations. Rule 5 recomputed live from Research Library on every Saturday run and republished in the brief. Writes gitted backup at `1-execution/areas/brady-os/hygiene-heidi-reports/YYYY-MM-DD.md` + Streaming Notes row + Routing Log row. Trigger: "run heidi", "hygiene check", "compliance check".
