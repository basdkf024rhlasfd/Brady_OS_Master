...
- **Claudine Scorecard:** `3-reference/skills/claudine-scorecard/SKILL.md` — Objective weekly scorecard replacing subjective Jarvis Score. 15 instrumented KPIs + North Star (Hands-Off Index). Every metric has a formula, data source, and scoring band pulled from Streaming Notes, git, Finn, Phil, Musashi. Runs weekly via weekly-sweep or on demand ("score claudine"). Auto-proposes fixes for any metric scoring <5.
- **Hygiene Heidi:** `0-agents/custom-built-agents/hygiene-heidi-SKILL.md` — Weekly Saturday 8 AM CT OS compliance check. Audits every agent against 4 canonical hygiene rules (objective scoring methodology, self-scoring, improvement-seeking, Streaming Notes purgatory ≤7 days). Red/amber/green brief + approval gates for all violations. Writes gitted backup at `1-execution/areas/brady-os/hygiene-heidi-reports/YYYY-MM-DD.md` + Streaming Notes row + Routing Log row. Trigger: "run heidi", "hygiene check", "compliance check".
