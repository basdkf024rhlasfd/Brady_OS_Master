...
#### 6. LinkedIn CRM & Network Data

Build a local CRM from LinkedIn network data for consulting pipeline, content targeting, and relationship tracking.

Primary recommendation:

- **OpenOutreach** — self-hosted LinkedIn automation with a built-in DjangoCRM web interface. Scrapes profiles, learns your ideal customer profile through embeddings, and manages multi-turn follow-up conversations. Best all-in-one option.

Alternative tools:

- **Skyvern** — general-purpose browser automation via Playwright and AI. Use if OpenOutreach is too opinionated or you want a fully custom scrape-to-database pipeline.
- **linkedin_scraper** — lightweight Python library for profile data extraction. Good for one-off pulls or prototyping.
- **drissbri/linkedin-scraper** — REST API (Selenium + FastAPI) for profile and company data. Good for building a custom extraction service.

Caveats:

- LinkedIn actively bans scrapers. Their 2026 detection is aggressive.
- LinkedIn ToS prohibits scraping. Use a dedicated account, not your primary profile.
- Prefer the official LinkedIn API where possible.
- Aligns with Brady OS stance: no full autonomous desktop control, prefer partial automation with human checkpoints.
- Human review required before any automated outreach. No autonomous messaging.
...
- <https://github.com/drissbri/linkedin-scraper>

### Pilot Only

#### 7. Open WebUI or LibreChat
...
LinkedIn (dedicated account)
        |
        v
OpenOutreach / Skyvern / scraper
        |
        +--> Local CRM (DjangoCRM or custom DB)
        +--> n8n (sync to Notion, content targeting)
```
...
- `OpenOutreach` or scraper: LinkedIn network data extraction and CRM
