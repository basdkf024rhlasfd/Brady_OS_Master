    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
      ...
      <div className="flex items-end justify-between">
        ...
        <div className="text-right">
          <p className="text-[10px] uppercase tracking-wider text-muted-foreground">Weekly spend</p>
      ...
      <div className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-4">
        ...
        {adhocItems.length > 0 ? (
          ...
          <p className="text-xs text-muted-foreground">Nothing needed beyond subscriptions.</p>
        )}
      </div>
...
function Th({
  ...
  const content = (
    <span className="inline-flex items-center gap-1">
      {children}
      {sortable && active && <span className="text-foreground">{dir === "asc" ? "↑" : "↓"}</span>}
    </span>
  );
  return (
    <th
    ...
    >
      {sortable ? (
        <button onClick={onClick} className="hover:text-foreground transition-colors">{content}</button>
      ) : (
        content
      )}
    </th>
